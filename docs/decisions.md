...
## 2026-09-18 — Run 1.1 close-out

- **Gate 1.1 closed.** Gary released a new app version with `write_publications` and re-approved the install; `pnpm seed --wipe-others` then published all 11 products and both collections to the four publications (Online Store, POS, Shop, Headless) and removed the 17 test products. The Storefront API returns all 11.
- **The seed log misread as data loss.** Every run starts by deleting the previous seed copies, and the log said only `deleted witness-tee`, which Gary read as the wipe removing the new products. Fixed: the two delete steps now say `removed previous seed copy` and `removed test data`, and the run ends with `store now has N product(s): X seeded, Y other`. Lesson: a destructive script's log must say which category each deletion belongs to and why it is safe.
